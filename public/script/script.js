...
/* Consultation modal: delegated open (replaces inline onclick=openConsultationModal) */
document.addEventListener('click', function (e) {
    const trigger = e.target.closest('[data-open-consult]');
    if (trigger) {
        e.preventDefault();
        if (typeof window.openConsultationModal === 'function') window.openConsultationModal(e);
    }
});


/* Client testimonials: "Read More" expander (page-specific, selector-guarded) */
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        const textCards = document.querySelectorAll('.testimonials-section-view .text-card');
        textCards.forEach(card => {
            const textEl = card.querySelector('.review-text');
            if (textEl && !card.querySelector('.read-more-btn') && textEl.scrollHeight > textEl.clientHeight) {
