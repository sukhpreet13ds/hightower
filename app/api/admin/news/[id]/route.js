...
export async function PUT(req, { params }) {
  ...
  const slug = customSlug ? slugify(customSlug) : (existing.slug || slugify(title) || `news-${id}`);
  ...
  await run(`
    UPDATE news SET slug = ?, title = ?, excerpt = ?, content = ?, image = ?, logo = ?, tags = ?, author = ?, meta_title = ?, meta_description = ?, published = ?
    WHERE id = ?
  `, [
    slug,
    title,
