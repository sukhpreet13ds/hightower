...
export async function POST(req) {
  ...
  const slug = slugify(customSlug || title) || `news-${Date.now()}`;
  ...
  const info = await run(`
    INSERT INTO news (slug, title, excerpt, content, image, logo, tags, author, meta_title, meta_description, published, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
    slug,
    title,
